import logging
import traceback
import uuid

from app.models.common.analysis import AnalysisStatus
from app.models.common.base import DataMartResourceLink, DataMartResourceLinkResponse
from app.models.land_change.tree_cover import (
    TreeCoverAnalytics,
    TreeCoverAnalyticsIn,
    TreeCoverAnalyticsResponse,
)
from app.use_cases.analysis.tree_cover.tree_cover_service import (
    TreeCoverService,
)
from fastapi import APIRouter, BackgroundTasks, HTTPException, Request
from fastapi import Response as FastAPIResponse
from fastapi.responses import ORJSONResponse

router = APIRouter(prefix="/tree_cover")


@router.post(
    "/analytics",
    response_class=ORJSONResponse,
    response_model=DataMartResourceLinkResponse,
    status_code=202,
)
def create(
    *,
    data: TreeCoverAnalyticsIn,
    request: Request,
    background_tasks: BackgroundTasks,
):
    try:
        service = TreeCoverService()
        background_tasks.add_task(service.do, data)

        link_url = request.url_for(
            "get_tree_cover_analytics_result", resource_id=data.thumbprint()
        )
        link = DataMartResourceLink(link=str(link_url))

        return DataMartResourceLinkResponse(data=link, status=AnalysisStatus.pending)
    except Exception as e:
        logging.error(
            {
                "event": "tree_cover_analytics_processing_failure",
                "severity": "high",  # Helps with alerting
                "error_type": e.__class__.__name__,  # e.g., "ValueError", "ConnectionError"
                "error_details": str(e),
                "stack_trace": traceback.format_exc(),
            }
        )
        raise HTTPException(status_code=500, detail="Internal Server Error")


@router.get(
    "/analytics/{resource_id}",
    response_class=ORJSONResponse,
    response_model=TreeCoverAnalyticsResponse,
    status_code=200,
)
async def get_tree_cover_analytics_result(
    resource_id: str,
    response: FastAPIResponse,
):
    # Validate UUID format
    try:
        uuid.UUID(resource_id)
    except ValueError:
        raise HTTPException(
            status_code=400, detail="Invalid resource ID format. Must be a valid UUID."
        )

    try:
        response.headers["Retry-After"] = "1"

        return TreeCoverAnalyticsResponse(
            data=TreeCoverAnalytics(
                status=AnalysisStatus.pending,
                message="Resource is still processing, follow Retry-After header.",
                result=None,
                metadata=None,
            ),
            status="success",
        )
    except Exception as e:
        logging.error(
            {
                "event": "tree_cover_analytics_resource_request_failure",
                "severity": "high",  # Helps with alerting
                "resource_id": resource_id,
                "resource_metadata": None,
                "error_type": e.__class__.__name__,  # e.g., "ValueError", "ConnectionError"
                "error_details": str(e),
                "traceback": traceback.format_exc(),
            }
        )
        raise HTTPException(status_code=500, detail="Internal server error")
