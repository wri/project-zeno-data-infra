...
AREA_SIZE_CATEGORIES = [
    (0, 500_000, "0_to_500k"),
    (500_000, 1_000_000, "500k_to_1M"),
    (1_000_000, 10_000_000, "1M_to_10M"),
    (10_000_000, 100_000_000, "10M_to_100M"),
    (None, None, "any_area"),
]
...
class IntenseApiUser(FastHttpUser):
    ...
    def test_request_analytics_and_wait_for_result(self):
        area_min, area_max, area_label = random.choice(AREA_SIZE_CATEGORIES)
        ...
        with measure_task_time(f"full_run_{run_id}_{area_label}"):
            ...
            def _get_aoi_ids(aoi_type):
                ...
                area_filter = (
                    f"and gfw_area__ha >= {area_min} and gfw_area__ha < {area_max}"
                ...
                if aoi_type == "admin":
                    ...
                    count_resp = requests.get(
                        f"https://data-api.globalforestwatch.org/dataset/birdlife_key_biodiversity_areas/latest/query?sql=select count(*) as cnt from data where sitlat < 70 and sitlat > -70 {area_filter}&x-api-key={API_KEY}"
                    ).json()
                    total = count_resp["data"][0]["cnt"]
                    max_offset = max(0, total - number_of_ids)
                    offset = random.randint(0, max(1, max_offset))
                    resp = requests.get(
                        f"https://data-api.globalforestwatch.org/dataset/birdlife_key_biodiversity_areas/latest/query?sql=select sitrecid from data where sitlat < 70 and sitlat > -70 {area_filter} limit {number_of_ids} offset {offset}&x-api-key={API_KEY}"
                    ).json()
                    aoi_ids = [str(row["sitrecid"]) for row in resp["data"]]
                    print("KBA AOI IDS", aoi_ids, area_label)
                elif aoi_type == "protected_area":
                    count_resp = requests.get(
                        f"https://data-api.globalforestwatch.org/dataset/wdpa_protected_areas/latest/query?sql=select count(*) as cnt from data where 1=1 {area_filter}&x-api-key={API_KEY}"
                    ).json()
                    total = count_resp["data"][0]["cnt"]
                    max_offset = max(0, total - number_of_ids)
                    offset = random.randint(0, max(1, max_offset))
                    ...
                    count_resp = requests.get(
                        f"https://data-api.globalforestwatch.org/dataset/landmark_ip_lc_and_indicative_poly/latest/query?sql=select count(*) as cnt from data where 1=1 {area_filter}&x-api-key={API_KEY}"
                    ).json()
                    total = count_resp["data"][0]["cnt"]
                    max_offset = max(0, total - number_of_ids)
                    offset = random.randint(0, max(1, max_offset))
                    resp = requests.get(
                        f"https://data-api.globalforestwatch.org/dataset/landmark_ip_lc_and_indicative_poly/latest/query?sql=select landmark_id from data where 1=1 {area_filter} limit {number_of_ids} offset {offset}&x-api-key={API_KEY}"
                    ).json()
                    aoi_ids = [str(row["landmark_id"]) for row in resp["data"]]
                    print("Indigenous AOI IDS", aoi_ids, area_label)
                return aoi_ids
            ...
            retry_count = 0
            group_name = (
                f"land_change:analytics::{aoi_type}::{endpoint}::{area_label}::resource"
            )
            # print(link)
                ...
                with self.rest(
                    ...
                    if "Retry-After" in response.headers:
                        ...
                        if response.status_code == 200:
                            ...
                            if status == "failed":
                                response.failure(
                                    f"Analysis failed for {endpoint} resource: {resource_id}"
                                )
                            else:
                                events.request.fire(
                                    request_type="CUSTOM",
                                    name=f"processing_time_{aoi_type}_{endpoint}_{area_label}",
