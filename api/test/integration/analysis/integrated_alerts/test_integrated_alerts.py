...
from app.routers.land_change.integrated_alerts import (
    ...
    return AnalysisService(
        ...
        analyzer=IntegratedAlertsAnalyzer(
            ...
            duckdb_query_service=DuckDbPrecalcQueryService(
                table_uri="s3://lcl-analytics/zonal-statistics/admin-integrated-alerts.parquet"
            ),
        ),
...
class TestIntegratedAlertsPostWithMultipleAdminAOIs:
            ...
            async with LifespanManager(app):
                async with AsyncClient(
                    ...
                    request = await client.post(
                        f"/v0/integrated_alerts/{ANALYTICS_NAME}/analytics",
    ...
    async def test_post_returns_resource_link(self, setup):
        ...
        assert (
            resource["data"]["link"]
            == f"http://testserver/v0/integrated_alerts/{ANALYTICS_NAME}/analytics/{analysis_params.thumbprint()}"
        )
...
class TestIntegratedAlertsPostWithKba:
    @pytest_asyncio.fixture
    async def setup(self):
        analytics_in = IntegratedAlertsAnalyticsIn(
