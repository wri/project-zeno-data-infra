...
@flow(
    ...
    log_prints=True,
    description=(
        "Compute DIST alerts areas by GADM admin levels 0, 1, and 2, as well as by "
        "various contextual layers, and run validation suite on the results."
    ),
)
def dist_alerts_flow(dist_version=None, overwrite=False, is_latest=False) -> list[str]:
    ...
    # The sub-flows share alert pixels extracted (and persisted) on first use.
    try:
        ...
        if not validate_natural_lands_result["validation_passed"]:
            raise ValueError(
                "DIST area by natural lands validation failed: "
                f"{validate_natural_lands_result.get('details', {})}"
            )
        ...
        if not validate_drivers_result["validation_passed"]:
            raise ValueError(
                "DIST area by drivers validation failed: "
                f"{validate_drivers_result.get('details', {})}"
            )
        ...
        if not validate_grasslands_result["validation_passed"]:
            raise ValueError(
                "DIST area by grasslands validation failed: "
                f"{validate_grasslands_result.get('details', {})}"
            )
        ...
        if not validate_land_cover_result["validation_passed"]:
            raise ValueError(
                "DIST area by land cover validation failed: "
                f"{validate_land_cover_result.get('details', {})}"
            )
