...
def setup_compute(datasets: Tuple[xr.Dataset, ...]) -> Tuple:
    ...
    return stages.setup_compute(datasets, expected_groups, "natural_lands")
...
def save_result(result_df: pd.DataFrame, dist_version: str):
    return stages.save_results(result_df, dist_version, "dist_alerts_by_natural_lands")


@flow(name="DIST alerts count by natural lands")
def dist_alerts_by_natural_lands_count(dist_zarr_uri: str, dist_version: str):
