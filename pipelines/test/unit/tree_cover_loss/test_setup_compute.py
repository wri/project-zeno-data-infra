...
from pipelines.tree_cover_loss.stages import setup_compute
...
def _create_mock_datasets(shape=(2, 2)):
    ...
    tcl = xr.DataArray(da.array(tcl_data, dtype=np.uint8), dims=["y", "x"], coords=coords)
    tcd = xr.DataArray(da.array(tcd_data, dtype=np.uint8), dims=["y", "x"], coords=coords)
    ifl = xr.DataArray(da.array(ifl_data, dtype=np.int16), dims=["y", "x"], coords=coords)
    drivers = xr.DataArray(da.array(drivers_data, dtype=np.int16), dims=["y", "x"], coords=coords)
    primary_forests = xr.DataArray(da.array(primary_forests_data, dtype=np.uint8), dims=["y", "x"], coords=coords)
    country = xr.DataArray(da.array(country_data, dtype=np.int16), dims=["y", "x"], coords=coords)
    region = xr.DataArray(da.array(region_data, dtype=np.uint8), dims=["y", "x"], coords=coords)
    subregion = xr.DataArray(da.array(subregion_data, dtype=np.int16), dims=["y", "x"], coords=coords)

    area_and_emissions = xr.Dataset({
        "area_ha": xr.DataArray(da.array(area_data, dtype=np.float32), dims=["y", "x"], coords=coords),
        "carbon__Mg_CO2e": xr.DataArray(da.array(carbon_data, dtype=np.float32), dims=["y", "x"], coords=coords),
    })

    return (tcl, area_and_emissions, tcd, ifl, drivers, primary_forests, country, region, subregion)
