...
from pipelines.afolu_vegetation.qc import qc_against_reference
...
def reference_totals():
    return pd.DataFrame(
        {
            "country": ["STP", "STP"],
            "veg_category": ["tree_loss", "trees_remaining_trees"],
...
def result_df(scale=1.0):
    # country-level rows (aoi_id without a dot) plus sub-admin rows that must be
    # ignored by the country-level QC.
    return pd.DataFrame(
        {
            "aoi_id": ["STP", "STP", "STP.1", "STP.1"],
            "aoi_type": ["admin"] * 4,
            "veg_category": [
                "tree_loss",
            ...
            "gross_emissions_MgCO2e": [9182.0 * scale, 1244.5 * scale, 999.0, 999.0],
            "gross_removals_MgCO2": [0.0, -11663944.0 * scale, 0.0, 0.0],
            "net_flux_MgCO2e": [9182.0 * scale, -11663944.0 * scale, 999.0, 999.0],
        }
    )
